...
class AddUpProductsToOrderPage_Controller extends Page_Controller
{
     ...
     *
     **/
    public function AddProductsToOrderRows()
    {
        ...
        for ($i = $startNumber ; $i < $this->rowNumbers; $i++) {
            if (!isset($savedValuesArray[$i])) {
                $savedValuesArray[$i] = array();
            }
            if (!isset($savedValuesArray[$i]["Name"])) {
                $savedValuesArray[$i]["Name"]= "";
            }
            if (!isset($savedValuesArray[$i]["Qty"])) {
                $savedValuesArray[$i]["Qty"]= 0;
            }
            if (!isset($savedValuesArray[$i]["BuyableClassNameAndID"])) {
                $savedValuesArray[$i]["BuyableClassNameAndID"]= 0;
            }
            if (!isset($savedValuesArray[$i]["Total"])) {
                $savedValuesArray[$i]["Total"]= 0;
            }
            $do = new DataObject();
            $do->RowNumber = $i;
